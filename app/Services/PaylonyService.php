...
class PaylonyService
{
    ...
    {
        $this->secretKey = config('services.paylony.secret_key', 'sk_live_2ppyvqwz3uybnk746z0wgu4ox21ietvhiwacwwa');
        $this->baseUrl = config('services.paylony.base_url', 'https://mainapi.paylony.com/api/v1');
        $this->timeout = config('services.paylony.timeout', 30);
    ...
    public function makePayout(Withdrawal $withdrawal): array
    {
        ...
        try {
            ...
            Log::info('paylony: Initiating makePayout', $payload);
            ...
            Log::notice('paylony: makePayout response received', [
            ...
            Log::info('paylony: makePayout response received', [
                'status_code' => $response->status(),
            ...
            Log::error('paylony: makePayout failed', [
    ...
    public function createVirtualAccount(User $user): array
    {
        ...
        try {
            ...
            $payload = [
                ...
                'provider' => env('PAYLONY_DEFAULT_BANK',"vfd"),
                'id' => $user->id
            ];

            Log::info('paylony: Initiating createVirtualAccount', $payload);
            ...
            Log::notice('paylony: createVirtualAccount response received', [
                'status_code' => $response->status(),
                'has_data' => $data
            ]);
            ...
            Log::info('paylony: createVirtualAccount response received', [
                'status_code' => $response->status(),
                'has_data' => isset($data['data'])
            ]);
            ...
            Log::error('paylony: createVirtualAccount failed', [
                'error' => $exception->getMessage(),
                'trace' => $exception->getTraceAsString()
            ]);
    ...
    public function createCustomer(User $user): string
    {
        try {
            ...
            Log::info('paylony: Initiating createCustomer', $payload);
            ...
            Log::notice('paylony: createCustomer response received', $data);
            ...
            Log::error('paylony: createCustomer validation failed', (array)$exception->getMessage());
    ...
    {
        try {
            Log::info('paylony: Initiating bank list retrieval');
            ...
            Log::info('paylony: API response received', [
                'status_code' => $response->status(),
                'has_data' => isset($data['data'])
            ]);
            ...
            Log::error('paylony: Bank list retrieval failed', [
                'error' => $exception->getMessage(),
                'trace' => $exception->getTraceAsString()
            ]);
    ...
    public function validateAccountName(string $accountNumber, string $bankCode, string $currency = 'NGN'): array
    {
        try {
            ...
            Log::info('paylony: Initiating account name validation', [
            ...
            Log::notice('paylony: Api response received', [
                'status_code' => $response->status(),
                'has_data' => $data,
                'account_number' => $this->maskAccountNumber($accountNumber)
            ]);
            ...
            Log::info('paylony: Account validation response received', [
                'status_code' => $response->status(),
                'has_data' => isset($data['data']),
                'account_number' => $this->maskAccountNumber($accountNumber)
            ]);
            ...
            Log::error('paylony: Account name validation failed', [
                'account_number' => $this->maskAccountNumber($accountNumber),
                'bank_code' => $bankCode,
                'error' => $exception->getMessage(),
                'trace' => $exception->getTraceAsString()
...
//        }
        ...
        Log::info('paylony: Account validation successful', [
            'account_number' => $this->maskAccountNumber($accountNumber),
    ...
    protected function processApiResponse(array $data): array
    {
        ...
        Log::info('paylony: Bank list retrieved successfully', [
    ...
    protected function validateConfiguration(): void
    {
        if (empty($this->secretKey)) {
            throw new Exception('paylony secret key is not configured');
        }

        if (empty($this->baseUrl)) {
            throw new Exception('paylony base URL is not configured');
        }

        if (!filter_var($this->baseUrl, FILTER_VALIDATE_URL)) {
            throw new Exception('paylony base URL is not valid');
        }
    }
